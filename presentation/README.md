...
1. Czym są szablony?
   ...
   - **Malejąca popularność**: Wraz z rosnącą popularnością frameworków renderujących dane po stronie klienta takich jak np. React czy Vue rola klasycznych szablonów serwerowych zmniejsza się.
...
2. Czym są silniki szablonów?
   - **Definicja**: Narzędzia lub biblioteki przetwarzające szablony w celu wygenerowania ostatecznego dokumentu na podstawie dostarczonych danych.
   - **Własna składnia**: Każdy silnik wprowadza swój własny zestaw konstrukcji takich jak pętle, warunki czy zmienne np. Handlebars, EJS oraz Pug.
   - **Obsługa danych**: Silniki umożliwiają przekazywanie danych do szablonów i ich wykorzystanie w czasie renderowania.
   - **Bezpieczeństwo**: Silniki szablonów same w sobie nie są podatne na ataki, podatności pojawiają się w wyniku błędów w ich wykorzystaniu.
   - **Popularność**: W przeszłości szeroko stosowane w aplikacjach internetowych, dziś często współpracują z technologiami renderowania po stronie klienta.
3. Czym jest podatność SSTI?
   - **Definicja**: Server Side Template Injection (SSTI) to podatność umożliwiająca użytkownikowi wprowadzenie własnej logiki do szablonu przetwarzanego po stronie serwera.
   ...
   - **Przykłady**: Możliwość wstrzyknięcia wyrażeń w silniku Handlebars (`{{ this.constructor }}`) lub wykonania poleceń w Jinja (Python).
...
5. Jak można wykorzystać podatność SSTI?
   ...
   - **Pozyskanie danych**: Możliwość wykradania poufnych informacji, takich jak dane użytkowników, konfiguracje czy zmienne środowiskowe.
   - **Utworzenie tylnych drzwi**: W niektórych przypadkach możliwe jest wprowadzenie trwałych zmian w aplikacji lub plikach serwera.
...
7. Jak prawidłowo używać silnika szablonów?
   ...
   - **Unikanie niektórych znaków**: Automatyczne lub ręczne stosowanie odpowiednich metod zabezpieczania czy też zamiany istotnych znaków dla wyjścia HTML, JSON itp.
...
8. Jak poprawnie zabezpieczać dane wejściowe?
   ...
   - **Wykorzystanie API**: Zamiast dynamicznego renderowania szablonów można przesyłać jedynie dane do klienta.
9. Jakie można wyciągnąć wnioski?
   ...
   - **Bezpieczeństwo priorytetem**: Podatności takie jak SSTI wynikają z błędów w przetwarzaniu danych wejściowych. Odpowiednie korzystanie z bibliotek, walidacje i filtrowanie danych są kluczowe dla bezpieczeństwa aplikacji.
   ...
   - **Przemyślane projektowanie aplikacji**: Renderowanie po stronie klienta (np. z wykorzystaniem frameworków takich jak React czy Svelte) może w wielu przypadkach zastąpić tradycyjne szablony serwerowe, eliminując ryzyko związane z podatnością SSTI.
